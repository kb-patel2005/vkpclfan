'use client'

import { motion } from "framer-motion";
import {
  Zap,
  Wrench,
  Thermometer,
  ShieldCheck,
  Network,
} from "lucide-react";

const first = () => (
  <svg width="24" height="30" viewBox="0 0 24 30" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M9.825 24.3L17.5875 15H11.5875L12.675 6.4875L5.7375 16.5H10.95L9.825 24.3ZM6 30L7.5 19.5H0L13.5 0H16.5L15 12H24L9 30H6Z" fill="#09273A" />
  </svg>

)

const second = () => (
  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M1.5 27.7588V23.2588H6.15L2.325 10.7338C1.65 10.3588 1.09375 9.80882 0.65625 9.08382C0.21875 8.35882 0 7.58382 0 6.75882C0 5.50882 0.4375 4.44632 1.3125 3.57132C2.1875 2.69632 3.25 2.25882 4.5 2.25882C5.475 2.25882 6.34375 2.54007 7.10625 3.10257C7.86875 3.66507 8.4 4.38382 8.7 5.25882H13.5V3.75882C13.5 3.33382 13.6437 2.97757 13.9312 2.69007C14.2188 2.40257 14.575 2.25882 15 2.25882C15.225 2.25882 15.4438 2.30882 15.6562 2.40882C15.8687 2.50882 16.05 2.65882 16.2 2.85882L18.75 0.458824C18.975 0.233824 19.2437 0.0900735 19.5562 0.0275735C19.8687 -0.0349265 20.175 0.00882353 20.475 0.158824L26.325 2.85882C26.625 3.00882 26.8313 3.22757 26.9438 3.51507C27.0562 3.80257 27.05 4.08382 26.925 4.35882C26.775 4.65882 26.5562 4.85257 26.2687 4.94007C25.9812 5.02757 25.7 5.00882 25.425 4.88382L20.025 2.40882L16.5 5.70882V7.80882L20.025 11.0338L25.425 8.55882C25.7 8.43382 25.9875 8.42132 26.2875 8.52132C26.5875 8.62132 26.8 8.80882 26.925 9.08382C27.075 9.38382 27.0875 9.67132 26.9625 9.94632C26.8375 10.2213 26.625 10.4338 26.325 10.5838L20.475 13.3588C20.175 13.5088 19.8687 13.5526 19.5562 13.4901C19.2437 13.4276 18.975 13.2838 18.75 13.0588L16.2 10.6588C16.05 10.8088 15.8687 10.9463 15.6562 11.0713C15.4438 11.1963 15.225 11.2588 15 11.2588C14.575 11.2588 14.2188 11.1151 13.9312 10.8276C13.6437 10.5401 13.5 10.1838 13.5 9.75882V8.25882H8.7C8.625 8.45882 8.54375 8.64632 8.45625 8.82132C8.36875 8.99632 8.25 9.18382 8.1 9.38382L15.6 23.2588H21V27.7588H1.5ZM4.5 8.25882C4.925 8.25882 5.28125 8.11507 5.56875 7.82757C5.85625 7.54007 6 7.18382 6 6.75882C6 6.33382 5.85625 5.97757 5.56875 5.69007C5.28125 5.40257 4.925 5.25882 4.5 5.25882C4.075 5.25882 3.71875 5.40257 3.43125 5.69007C3.14375 5.97757 3 6.33382 3 6.75882C3 7.18382 3.14375 7.54007 3.43125 7.82757C3.71875 8.11507 4.075 8.25882 4.5 8.25882ZM9.225 23.2588H12.15L5.7 11.2588C5.7 11.2588 5.675 11.2588 5.625 11.2588C5.575 11.2588 5.55 11.2588 5.55 11.2588L9.225 23.2588Z" fill="#09273A" />
  </svg>

);

const third = () => (
  <svg width="27" height="27" viewBox="0 0 27 27" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M15 12V9H22.5V12H15ZM15 6V3H27V6H15ZM7.5 27C5.425 27 3.65625 26.2687 2.19375 24.8062C0.73125 23.3438 0 21.575 0 19.5C0 18.3 0.2625 17.1812 0.7875 16.1437C1.3125 15.1062 2.05 14.225 3 13.5V4.5C3 3.25 3.4375 2.1875 4.3125 1.3125C5.1875 0.4375 6.25 0 7.5 0C8.75 0 9.8125 0.4375 10.6875 1.3125C11.5625 2.1875 12 3.25 12 4.5V13.5C12.95 14.225 13.6875 15.1062 14.2125 16.1437C14.7375 17.1812 15 18.3 15 19.5C15 21.575 14.2687 23.3438 12.8062 24.8062C11.3438 26.2687 9.575 27 7.5 27ZM3 19.5H12C12 18.775 11.8438 18.1 11.5312 17.475C11.2188 16.85 10.775 16.325 10.2 15.9L9 15V4.5C9 4.075 8.85625 3.71875 8.56875 3.43125C8.28125 3.14375 7.925 3 7.5 3C7.075 3 6.71875 3.14375 6.43125 3.43125C6.14375 3.71875 6 4.075 6 4.5V15L4.8 15.9C4.225 16.325 3.78125 16.85 3.46875 17.475C3.15625 18.1 3 18.775 3 19.5Z" fill="#09273A" />
  </svg>

);

const fourth = () => (
  <svg width="33" height="32" viewBox="0 0 33 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M11.4 31.5L8.55 26.7L3.15 25.5L3.675 19.95L0 15.75L3.675 11.55L3.15 6L8.55 4.8L11.4 0L16.5 2.175L21.6 0L24.45 4.8L29.85 6L29.325 11.55L33 15.75L29.325 19.95L29.85 25.5L24.45 26.7L21.6 31.5L16.5 29.325L11.4 31.5ZM12.675 27.675L16.5 26.025L20.4 27.675L22.5 24.075L26.625 23.1L26.25 18.9L29.025 15.75L26.25 12.525L26.625 8.325L22.5 7.425L20.325 3.825L16.5 5.475L12.6 3.825L10.5 7.425L6.375 8.325L6.75 12.525L3.975 15.75L6.75 18.9L6.375 23.175L10.5 24.075L12.675 27.675ZM14.925 21.075L23.4 12.6L21.3 10.425L14.925 16.8L11.7 13.65L9.6 15.75L14.925 21.075Z" fill="#09273A" />
  </svg>

);

const features = [
  {
    icon: first,
    title: "Energy Efficient",
    desc: "Advanced EC motors reduce power consumption by up to 45% compared to traditional AC units.",
  },
  {
    icon: second,
    title: "Heavy Duty",
    desc: "Machined from aerospace-grade aluminum and marine-grade stainless steel for zero-failure lifespans.",
  },
  {
    icon: third,
    title: "Thermal Control",
    desc: "Integrated smart sensors that auto-adjust RPM based on real-time ambient temperature and humidity.",
  },
  {
    icon: fourth,
    title: "Reliability",
    desc: "Every unit undergoes a 48-hour extreme stress test before leaving our precision engineering facility.",
  },
];

export function Advantage() {
  return (
    <section className="bg-[#F8F9FA] px-5 py-8 sm:px-6 sm:py-10 lg:px-0 lg:py-16">
      <div className="mx-auto mt-8 grid max-w-7xl grid-cols-12 gap-8 sm:mt-10 lg:mt-14 lg:gap-6">

        {/* ================= LEFT ================= */}
        <motion.div
          initial={{ x: -100, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1 }}
            viewport={{ once: true, amount: 0.3 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="col-span-12 lg:col-span-6"
        >
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {features.map((item, i) => {
              const Icon = item.icon;
              return (
                <div
                  key={i}
                  className={`
                    flex flex-col gap-3
                    rounded-[24px]
                    bg-white
                    p-6
                    shadow-sm
                    transition
                    duration-300
                    hover:shadow-xl
                    sm:p-7
                    lg:gap-4
                    lg:px-[32px]
                    ${i % 2 === 1
                      ? "lg:pt-[88px] lg:pb-[48px]"
                      : "lg:pt-[32px] lg:pb-[88px]"
                    }
                  `}
                >
                  <div className="shrink-0">
                    <Icon />
                  </div>
                  <h3 className="pt-2 font-sora text-[20px] font-semibold leading-7 text-[#09273A] sm:text-[22px] sm:leading-8 lg:pt-3 lg:text-2xl lg:leading-8">
                    {item.title}
                  </h3>
                  <p className="font-inter text-[14px] font-normal leading-6 text-[#5D5D5D] sm:text-[15px] lg:text-[16px]">
                    {item.desc}
                  </p>
                </div>
              );
            })}
          </div>
        </motion.div>

        {/* ================= RIGHT ================= */}
        <motion.div
          initial={{ x: 100, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.8, ease: "easeOut" }}
          className="col-span-12 flex flex-col justify-center gap-3 lg:col-span-6 lg:gap-4 lg:pl-12"
        >
          <span className="text-[11px] font-semibold uppercase tracking-[0.6px] text-[#09273A] sm:text-xs">
            The VENAIRA Advantage
          </span>

          <h2 className="font-sora text-[28px] font-bold leading-9 tracking-[-0.2px] text-[#0B2B45] sm:text-[34px] sm:leading-[42px] lg:text-[40px] lg:leading-[48px] lg:tracking-[-0.4px]">
            Hardware Built For The Decade, Software Built For Performance.
          </h2>

          <p className="w-full py-2 font-inter text-[15px] font-normal leading-6 text-[#5D5D5D] sm:text-[16px] sm:leading-7 lg:w-[516px] lg:py-4 lg:text-[18px]">
            We don't just move air. We optimize environments. Our systems are the heartbeat of modern manufacturing, ensuring safety, comfort, and productivity.
          </p>

          <div className="flex items-start gap-3 p-2 sm:gap-4 sm:p-3 lg:p-4">
            <div className="flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-full bg-[#09273A38] sm:h-[48px] sm:w-[48px]">
              {/* Your SVG icon */}
              <svg
                width="24"
                height="23"
                viewBox="0 0 24 23"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M6 23C5.16667 23 4.45833 22.7083 3.875 22.125C3.29167 21.5417 3 20.8333 3 20C3 19.1667 3.29167 18.4583 3.875 17.875C4.45833 17.2917 5.16667 17 6 17C6.23333 17 6.45 17.025 6.65 17.075C6.85 17.125 7.04167 17.1917 7.225 17.275L8.65 15.5C8.18333 14.9833 7.85833 14.4 7.675 13.75C7.49167 13.1 7.45 12.45 7.55 11.8L5.525 11.125C5.24167 11.5417 4.88333 11.875 4.45 12.125C4.01667 12.375 3.53333 12.5 3 12.5C2.16667 12.5 1.45833 12.2083 0.875 11.625C0.291667 11.0417 0 10.3333 0 9.5C0 8.66667 0.291667 7.95833 0.875 7.375C1.45833 6.79167 2.16667 6.5 3 6.5C3.83333 6.5 4.54167 6.79167 5.125 7.375C5.70833 7.95833 6 8.66667 6 9.5C6 9.53333 6 9.56667 6 9.6C6 9.63333 6 9.66667 6 9.7L8.025 10.4C8.35833 9.8 8.80417 9.29167 9.3625 8.875C9.92083 8.45833 10.55 8.19167 11.25 8.075V5.9C10.6 5.71667 10.0625 5.3625 9.6375 4.8375C9.2125 4.3125 9 3.7 9 3C9 2.16667 9.29167 1.45833 9.875 0.875C10.4583 0.291667 11.1667 0 12 0C12.8333 0 13.5417 0.291667 14.125 0.875C14.7083 1.45833 15 2.16667 15 3C15 3.7 14.7833 4.3125 14.35 4.8375C13.9167 5.3625 13.3833 5.71667 12.75 5.9V8.075C13.45 8.19167 14.0792 8.45833 14.6375 8.875C15.1958 9.29167 15.6417 9.8 15.975 10.4L18 9.7C18 9.66667 18 9.63333 18 9.6C18 9.56667 18 9.53333 18 9.5C18 8.66667 18.2917 7.95833 18.875 7.375C19.4583 6.79167 20.1667 6.5 21 6.5C21.8333 6.5 22.5417 6.79167 23.125 7.375C23.7083 7.95833 24 8.66667 24 9.5C24 10.3333 23.7083 11.0417 23.125 11.625C22.5417 12.2083 21.8333 12.5 21 12.5C20.4667 12.5 19.9792 12.375 19.5375 12.125C19.0958 11.875 18.7417 11.5417 18.475 11.125L16.45 11.8C16.55 12.45 16.5083 13.0958 16.325 13.7375C16.1417 14.3792 15.8167 14.9667 15.35 15.5L16.775 17.25C16.9583 17.1667 17.15 17.1042 17.35 17.0625C17.55 17.0208 17.7667 17 18 17C18.8333 17 19.5417 17.2917 20.125 17.875C20.7083 18.4583 21 19.1667 21 20C21 20.8333 20.7083 21.5417 20.125 22.125C19.5417 22.7083 18.8333 23 18 23C17.1667 23 16.4583 22.7083 15.875 22.125C15.2917 21.5417 15 20.8333 15 20C15 19.6667 15.0542 19.3458 15.1625 19.0375C15.2708 18.7292 15.4167 18.45 15.6 18.2L14.175 16.425C13.4917 16.8083 12.7625 17 11.9875 17C11.2125 17 10.4833 16.8083 9.8 16.425L8.4 18.2C8.58333 18.45 8.72917 18.7292 8.8375 19.0375C8.94583 19.3458 9 19.6667 9 20C9 20.8333 8.70833 21.5417 8.125 22.125C7.54167 22.7083 6.83333 23 6 23ZM3 10.5C3.28333 10.5 3.52083 10.4042 3.7125 10.2125C3.90417 10.0208 4 9.78333 4 9.5C4 9.21667 3.90417 8.97917 3.7125 8.7875C3.52083 8.59583 3.28333 8.5 3 8.5C2.71667 8.5 2.47917 8.59583 2.2875 8.7875C2.09583 8.97917 2 9.21667 2 9.5C2 9.78333 2.09583 10.0208 2.2875 10.2125C2.47917 10.4042 2.71667 10.5 3 10.5ZM6 21C6.28333 21 6.52083 20.9042 6.7125 20.7125C6.90417 20.5208 7 20.2833 7 20C7 19.7167 6.90417 19.4792 6.7125 19.2875C6.52083 19.0958 6.28333 19 6 19C5.71667 19 5.47917 19.0958 5.2875 19.2875C5.09583 19.4792 5 19.7167 6 21ZM12 4C12.2833 4 12.5208 3.90417 12.7125 3.7125C12.9042 3.52083 13 3.28333 13 3C13 2.71667 12.9042 2.47917 12.7125 2.2875C12.5208 2.09583 12.2833 2 12 2C11.7167 2 11.4792 2.09583 11.2875 2.2875C11.0958 2.47917 11 2.71667 11 3C11 3.28333 11.0958 3.52083 11.2875 3.7125C11.4792 3.90417 11.7167 4 12 4ZM12 15C12.7 15 13.2917 14.7583 13.775 14.275C14.2583 13.7917 14.5 13.2 14.5 12.5C14.5 11.8 14.2583 11.2083 13.775 10.725C13.2917 10.2417 12.7 10 12 10C11.3 10 10.7083 10.2417 10.225 10.725C9.74167 11.2083 9.5 11.8 9.5 12.5C9.5 13.2 9.74167 13.7917 10.225 14.275C10.7083 14.7583 11.3 15 12 15ZM18 21C18.2833 21 18.5208 20.9042 18.7125 20.7125C18.9042 20.5208 19 20.2833 19 20C19 19.7167 18.9042 19.4792 18.7125 19.2875C18.5208 19.0958 18.2833 19 18 19C17.7167 19 17.4792 19.0958 17.2875 19.2875C17.0958 19.4792 17 19.7167 18 21ZM21 10.5C21.2833 10.5 21.5208 10.4042 21.7125 10.2125C21.9042 10.0208 22 9.78333 22 9.5C22 9.21667 21.9042 8.97917 21.7125 8.7875C21.5208 8.59583 21.2833 8.5 21 8.5C20.7167 8.5 20.4792 8.59583 20.2875 8.7875C20.0958 8.97917 20 9.21667 20 9.5C20 9.78333 20.0958 10.0208 20.2875 10.2125C20.4792 10.4042 20.7167 10.5 21 10.5Z"
                  fill="#09273A"
                />
              </svg>
            </div>
            <div className="min-w-0">
              <h4 className="text-[14px] font-semibold leading-5 text-[#09273A] sm:text-[15px] sm:leading-6 lg:text-[16px]">
                Cloud Management Integrated
              </h4>
              <p className="mt-1 text-[12px] font-normal leading-5 text-[#5D5D5D] sm:text-[13px] lg:text-sm">
                Control entire fleets from a single terminal.
              </p>
            </div>
          </div>
        </motion.div>
      </div>
    </section>
  );
}